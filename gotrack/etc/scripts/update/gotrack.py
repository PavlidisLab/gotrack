...
class GOTrack:
    ...
    def _retrieve_id_map_for_edition(self, species_id, edition_id, cursor):
        ...
        id_map = {(db, db_object_id): id for id, db, db_object_id in cursor.fetchall()}
    ...
    def insert_annotations(self, species_id, edition_id, date, gpi_data, gpa_data, cursor=None):
        ...
        def flatten_and_id_synonyms(synonyms):
            for row in synonyms:
    ...
    @transactional
    def stage_accession_history_table(self, data, cursor=None):
        ...
        if cnt == 0:
            raise (ValueError("accession history file is either empty or malformed, code might need to be altered to "
...
# except Exception as inst:
#     log.error('Error on sp: %s, ed: %s, date: %s, %s', species_id, edition_id, date.strftime('%Y-%m-%d'), inst)
#     cursor.rollback()
#     raise
# finally:
#     if cursor:
